...
行情板块新增指标继续按独立脚本维护：涨停观察表使用 `scripts/update_limit_up_tables.py`，TMT/红利低波成交额占比使用 `scripts/update_theme_amount_share.py`，全市场成交额变化使用 `scripts/update_market_turnover.py`。港股板块与南向资金统一使用 `scripts/update_hk_dashboard.py` 从 Wind 金融能力取数，禁止再用旧南向脚本覆盖 `southbound_flow.csv`。不要通过对话搬运逐股大表；只保留前十汇总表、主题成交额时间序列和南向资金汇总序列。
